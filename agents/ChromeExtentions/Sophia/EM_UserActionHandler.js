...
//    consoleLog("Has object mutator support? " + !!window.MutationObserver, verbose);
    ...
    var reportIframeLogs = true,
        ...
        domContentLoaded = false, unloadCalled = false, jsBridge = window.__eumRumService.jsBridge, addedWorkaround = false, isIframe = false,
        registeredTargets = [],
        lastCallback = {};
...
//            consoleLog("pop from: xhr.constructor.prototype.abort", debug);
//            this.__eventThread.pop({isCancellation: true});
//            if (this.__eventThread.d3) consoleLog('Popped from abort xhr, queue: ' + this.__eventThread.queue, debug);
//
//        }
//        oldAbort();
//    };
...
//        consoleLog("Updating xhr report object " + this._eumNetworkObject.msgId + " with request header size " + this._eumNetworkObject.requestSize, debug);
//
//        //invoke the original (real) setRequestHeader:
//        try {
//            origSetRequestHeaders.apply(this, arguments);
//        } catch (e) {
//            consoleLog("Error in set request headers xhr (in original code)", info);
//        }
//    };
...
//        consoleLog("Qualifying url " + url, debug);
//        var qualifyUrl = getFullQualifyURL(url);
//        consoleLog("Qualified url " + qualifyUrl, debug);
...
//            consoleLog("xhr: " + this._eumNetworkObject.msgId + " reqSize:" + reqSize, debug);
//            this._eumNetworkObject.url = qualifyUrl;
//            this._eumNetworkObject.requestSize = reqSize;
//        }
//        else {
//            consoleLog("url " + url + " is local, skipping.", debug);
//        }
//        this._eumNetworkObject.method = method;
//        try {
//            oldOpen.apply(this, arguments);
//        } catch (e) {
//            consoleLog("Error in opening xhr (in original code)", info);
//        }
//    };
...
//        consoleLog("In wrapReadyStateFunction - XHR, URL: " + xhrObject.__eumUrl, debug);
...
//        if (!thread) {
//            consoleLog("In wrapReadyStateFunction - XHR, URL: " + xhrObject.__eumUrl + " ,not connected to thread.", debug);
//            xhrObject._wrappedReady = true;
//            return;
//        }
//        consoleLog("In wrapReadyStateFunction - XHR, URL: " + xhrObject.__eumUrl + " ,threadid:" + thread.id, debug);
...
//            consoleLog("In wrapReadyStateFunction.increaseCounter ,threadid:" + thread.id, debug);
//            //set timeout for XHR of 30 secs (to be in line with android native instrumentation)
//            xhr.__eumTimeout = oldSetTimeout(function () {
//                consoleLog("pop from: wrapReadyStateFunction->increaseCounter", debug);
//                thread.pop();
...
//            if (xhr.__eumTimeout) {
//                consoleLog("pop from: wrapReadyStateFunction->decreaseCounter", debug);
//                thread.pop();
//                oldClearTimeout(xhr.__eumTimeout);
//                delete xhr.__eumTimeout;
//            }
//        }
//
...
//            if (thread.d3) consoleLog("Added task at ready state " + thread.toString(), verbose);
//            xhrObject._wrappedReady = true;
...
//                            }
//                            consoleLog("xhr: " + xhrObject._eumNetworkObject.msgId + " responseLength " + responseLength, debug);
...
//                        doCallback(xhrObject, oldReadyState, 'xhr', arguments, thread);
//                    } catch (e) {
//                        consoleLog('Script error in original callback' + e, info);
//                    }
//                    //if it's at ready state 4, reduce queue by 1 and close thread
//                    if (xhrObject.readyState === 4) {
//                        decreaseCounter(thread, xhrObject);
//                        if (thread.d3) consoleLog("Popped after ready state queue: " + thread.queue, verbose);
//                        closeThread();
...
//                decreaseCounter(thread, xhrObject);
//                if (thread.d3) consoleLog("Popped after ready state (couldn't wrap) queue: " + thread.queue, verbose);
//            }
//            //responding to xhr can be via onload or on ready state change, if it's by onload, wrap it instead
//        } else if (oldOnLoad) {
//            increaseCounter(thread, xhrObject);
//            if (thread.d3) consoleLog("Added task at ready state ON LOAD " + thread.toString(), verbose);
//            xhrObject._wrappedReady = true;
//            xhrObject.onload = function () {
//                setThread(thread);
//                try {
//                    doCallback(xhrObject, oldOnLoad, 'xhr', arguments, thread);
//                } catch (e) {
//                    consoleLog('Script error in original callback' + e, info);
//                }
//                decreaseCounter(thread, xhrObject);
//                if (thread.d3) consoleLog("Popped after ready state on load " + thread.toString(), verbose);
//                closeThread();
//            };
//        }
...
//                setThread(thread);
//                try {
//                    doCallback(xhrObject, oldOnError, 'xhr', arguments, thread);
//                } catch (e) {
//                    consoleLog('Script error in original callback' + e, info);
//                }
...
//                if (thread.d3) consoleLog("Popped after ready state error: " + thread.toString(), verbose);
//                closeThread();
//            };
//        }
//    }
...
//        if ((currentEventThread || documentLoadEvent) && !ignore) {
//            self.__eventThread = currentEventThread || documentLoadEvent;
//            consoleLog("In XHR send, URL: " + self.__eumUrl + " ,found open thread: " + self.__eventThread.toString(), debug);
//        }
...
//        } catch (e) {
//            consoleLog("Error in original send " + e, info);
//        }
//    };
    ...
    function beginUserAction(thread) {
        var id = Math.floor(Math.random() * 5678910);
        thread.nativeEventId = id;
        var nameResolve = thread.nameResolve;
        var gestureName = '',
        ...
        if (nameResolve) {
            gestureName = 'tap';
            controlType = nameResolve.controlType;
            contextName = nameResolve.context;
            contextId = nameResolve.contextId;
            controlName = nameResolve.controlName;
            controlId = nameResolve.controlId;
            gestureProp = !nameResolve.gestureValue ? "-" : !nameResolve.gestureValue;
        }
        var params = {
                message: {
                    id: id, 
        ...
        jsBridge('beginUserAction', params);
    }
...
//            window.addEventListener("deviceready", handleDocumentLoad, false);
        ...
        beforeUnloadHandler = function (event) {
            if (beforeUnloadDone) {
                consoleLog("BeforeUnload was already triggered, do nothing", info);
                return;
            }

            //if unload was triggered programatically. This didn't actually happened. This is just in case handling.
            if (inEventCallback) {
                consoleLog("Unload was triggered not through browser, do nothing", info);
                return;
            }
            consoleLog('In unload event', debug);
            var last = lastEvent || {}, eventToSave, unloadSticky = 1000, now = dateNow();
            //set unload called to true - so it won't report events before it's time
            unloadCalled = true;
            var thread = last.thread || documentLoadEvent;
            //The unload event may come while a thread is opened or may come in delay to the action that caused it,
            // thus the current thread will be closed.
            //In order to know to hook it to the user action we check the last action/async operation that happened and if it occurred in
            //the timeframe of one second we assume this was the user action that caused it,
            //save event id from EumCurrentUserActionId, and check on loading if it already exists on saved, do not get new one
            if (thread && (thread.lastPop + unloadSticky >= now || thread.startTime + unloadSticky >= now)) {
                consoleLog("Found last event of type: " + thread.type + " that occurred " + (dateNow() - thread.startTime) + " ms before unload", debug);
                eventToSave = {
                    message: {
                        type: thread.type,
                        startTime: thread.startTime,
                        target: 'location.change',
                        nameResolve: thread.nameResolve,
                        nativeEventId: thread.nativeEventId,
                        eventChain: thread.eventChain
                    },
                    type: "UI",
                    timeStamp: new Date().getTime()
                };
                consoleLog('Saving unload and event with timestamp: ' + eventToSave.startTime, debug);
                //save the event on Java side
                var eventToSaveStr = JSON.stringify(eventToSave);
                consoleLog("saving event: " + eventToSaveStr, debug);
                jsBridge("saveEvent", eventToSave);
                var urlToSave = {type: "fl"};
//                jsBridge("saveUrl", JSON.stringify(urlToSave));
            } else {
                //This case is when we could not connect the unload event to any action.
                //this may happen if a native code initiated navigation on existing webview
                //in this case we do nothing.
                consoleLog('Got unload event that could not be connected to any event. Ignoring it. probably caused by native code', debug);
            }
            //If document load event still open, report it.
            if (documentLoadEvent) {
                consoleLog('In unload, found an open documentLoadEvent. Report it.', debug);
                doReportProbe(documentLoadEvent);
            }
            //make sure to flush the log before unload
            consoleLog('Flush log before unload', debug);
            window.__eumRumService.flushLog(true);
            beforeUnloadDone = true;

        };

        //register the unload event
        //In the unload event we save the current action/event to the java so we will get it in the new page
        //(in iOS the beforeunload is not working so using pagehide instead)
        windowOldEventListenerAdd.call(window, 'beforeunload', beforeUnloadHandler);
        windowOldEventListenerAdd.call(window, 'pagehide', beforeUnloadHandler);
