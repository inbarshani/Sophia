...
function runTests() {
    //console.log('get tests by type: ' + config.sophia.searchTypes.FLOWS);
    tests_queries.getTestsByType('' + config.sophia.searchTypes.FLOWS, '',
        function(err, tests) {
            ...
            neo4j_queries.doesPathExists(currentNodes, idolResultNodes, isFirstQuery,
                function(err, paths_to_nodes) {
                    ...
                    if (!err && paths_to_nodes) {
                        ...
                        paths_to_nodes = [];
                    if (currentQuery.type == config.sophia.queryTypes.QUERY)
...
function saveFlowResults(testId, results) {
    ...
    running--;
    tests_queries.saveTestRun(testId, config.sophia.testRunTypes.SCHEDULER, results);
}
