...
var queries = {
    ...
    saveCapturedTestHistory: function(testId, nodeId, startTimestamp, callback) {
            db.run("INSERT INTO SOP_CAPTURED_TESTS_HISTORY(ID, START_TIMESTAMP,NODE_ID)" +
                " VALUES ($id, $start, $node)", {
                    $id: testId,
                    $node: nodeId,
        ...
        }, // saveCapturedTestHistory

    updateCapturedTestHistory: function(testId, endTimestamp, callback) {
            db.run("UPDATE SOP_CAPTURED_TESTS_HISTORY" +
        ...
        }, // updateCapturedTestHistory

    getCapturedTestHistory: function(numOfTestsLimit, callback) {
        var tests_history = [];
        var query = "SELECT * FROM SOP_CAPTURED_TESTS_HISTORY" +
                " LIMIT $limit";
        db.all(query, {
                    $limit: numOfTestsLimit
                },
                function(err, rows) {
                    //console.log('db returned with tests by type: '+type);
                    if (err) {
                        if (callback) {
                            console.log('error in getCapturedTestHistory: ' + query + '\nError: ' + err);
                            callback(err);
                        }
                    } else {
                        for (var i = 0; i < rows.length; i++) {
                            tests_history.push({
                                id: rows[i].ID,
                                start: rows[i].START_TIMESTAMP,
                                end: rows[i].END_TIMESTAMP,
                                node_id: rows[i].NODE_ID
                            });
                        }
                        if (callback)
                            callback(null, tests_history);
                    }
                });
        }, // getCapturedTestHistory
