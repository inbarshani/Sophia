...
var queries = {
	...
	importScheme: function(){
		fs.readFile(sophia_consts.TESTS_DB_SCHEME_FILE, 'utf-8', function(err, data){
			if (err)
                    ...
                    if (err) {
                        ...
                        db.all("SELECT ID, QUERY_TEXT, QUERY_TYPE FROM SOP_QUERY WHERE TEST_ID=? ORDER BY POSITION", id, function(err, rows) {
                            if (err) {
                                ...
                                db.get("SELECT ID FROM SOP_RUN WHERE TEST_ID=? ORDER BY CREATED DESC", id, function(err, row) {
                                    if (err) {
                                        if (callback)
                                        {
                                        ...
                                        if (row)
                                        {
                                            db.all("SELECT QUERY_ID, RESULTS_COUNT FROM SOP_RUN_QUERY WHERE RUN_ID=? ORDER BY QUERY_ID", row.ID, function(err, rows) {
                                                if (err) {
    ...
    saveTestRun: function(testId, runType, queryRunsArray, callback) {
            ...
            db.run("INSERT INTO SOP_RUN(CREATED, TEST_ID, TYPE) VALUES ($created, $test, $type)", {
                $created: created,
                $test: testId,
                $type: runType
            }, function(err) {
