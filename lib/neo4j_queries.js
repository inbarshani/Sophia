...
var queries = {
				...
				if (results) {
		            results.map(function(result) {
		            	...
		            	result.b.map(function(b) {
		            		bbNodes.push({id:'' + b.id, type: b._data.metadata.labels[0]});
		            	});
		                backbone_nodes.push({test: {id: result.start.id, timestamp: result.start.data.timestamp}, bbNodes: bbNodes});
		            });
		            if (callback)
	...
	getNearestScreens: function(node_ids, callback){
	    var query = "MATCH node where id(node) IN {node_ids}"+
	    	...
	    	"*0.."+sophia_consts.MAX_PATH_LENGTH+"]->node" +
