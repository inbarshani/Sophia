...
var queries = {
	...
	doesPathExit: function(startNodes, destinationNodes, callback)
	{
		...
		if (startNodes.length > 0)
			startNodesWhere = "id(start) IN [" + startNodes.join(',') + "]";
		else
			startNodesWhere = "ANY(label in labels(start) WHERE label='Test')";
	...
	linkNode: function(isBackbone, node_id, prev_node_id, next_node_id, shouldLinkToPrev, callback) {
