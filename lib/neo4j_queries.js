...
var queries = {
	...
	getNodesStats: function(currentNodes, callback) {
	    ...
	    var query = "MATCH start-[:"+sophia_consts.dataLinkType+"]->next" +
	            " WHERE start.test_id=next.test_id" + 
	            " AND id(start) IN [" + currentNodes.join(',') + "]"+
	...
	getNearestScreens: function(node_ids, callback){
	    var query = "MATCH node where id(node) IN {node_ids}"+
	    	...
	    	"*0.."+sophia_consts.MAX_PATH_TO_SCREEN_LENGTH+"]->node" +
			" OPTIONAL MATCH pathToNextScreen=node-[:"+sophia_consts.dataLinkType+
			"*0.."+sophia_consts.MAX_PATH_TO_SCREEN_LENGTH+"]->(next:SCREEN)" +
