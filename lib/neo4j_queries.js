...
var queries = {
    ...
    getNearestData: function(node_ids, data_type, callback) {
        var query = "MATCH node where id(node) IN {node_ids}" +
            " OPTIONAL MATCH pathToPrevData=(prev:"+data_type+")-[:" + sophia_config.dataLinkType +
            "*0.." + sophia_config.MAX_PATH_TO_DATA_LENGTH + "]->node" +
            " OPTIONAL MATCH pathToNextData=node-[:" + sophia_config.dataLinkType +
            "*0.." + sophia_config.MAX_PATH_TO_DATA_LENGTH + "]->(next:"+data_type+")" +
            " RETURN COLLECT(DISTINCT prev.timestamp) AS PrevDataTimestamps," +
            " COLLECT(DISTINCT next.timestamp) AS NextDataTimestamps," +
            " COLLECT(DISTINCT id(prev)) AS PrevDataIDs," +
            " COLLECT(DISTINCT id(next)) AS NextDataIDs";
        var params = {
            node_ids: node_ids.map(Number)
        };

        console.log(' [****] Get nearest '+data_type+' by node_id query:\n' + query +
            '\nwith params: ' + JSON.stringify(params));
        db.cypher({
            query: query,
            params: params
            ...
            if (err) {
                ...
                //console.log(require('util').inspect(results, { showHidden: true, depth: 4 }));
                if (callback && results && results.length > 0 && results[0]) {
                    console.log(' [****] Get nearest '+data_type+' by node_id query: ' + query + '\n' +
                        'returned ' + results.length + ' results');
                    callback(results[0].PrevDataTimestamps, 
                        results[0].NextDataTimestamps, 
                        results[0].PrevDataIDs, 
                        results[0].NextDataIDs);
                } else if (callback)
                    callback(new Error('failed to get a new node\'s location'));
                else
                    throw new Error('no callback');
