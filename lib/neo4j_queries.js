//var config.sophia = require('./config.sophia');
...
var queries = {
	...
	doesPathExists: function(currentStartNodes, destinationNodes, isFirstQuery, callback)
	{
		...
		if (currentStartNodes && currentStartNodes.length>config.sophia.maxResults)
			currentStartNodes.length = config.sophia.maxResults;
		if (destinationNodes && destinationNodes.length>config.sophia.maxResults)
			destinationNodes.length = config.sophia.maxResults;
		...
		if (isFirstQuery)
		{
			startNode = "(start:"+config.sophia.backboneRoot+")";
		}
		else
		{
	    ...
	    var query = "MATCH path=shortestPath("+startNode+"-[*.."+config.sophia.MAX_PATH_LENGTH+"]->destination)" +
	            " WHERE " +
	        ...
	        if (err) {
	            ...
	            results.map(function(result)
	            {
	            	...
	            	for (var i=0;i<path_nodes.length;i++)
	            	{
	            		...
	            		if (config.sophia.backboneTypes.indexOf(node_type) >= 0)
	...
	countPaths: function(nodesGroupA, nodesGroupB, callback)
	{
		...
		if (nodesGroupA && nodesGroupA.length>config.sophia.maxResults)
			nodesGroupA.length = config.sophia.maxResults;
		if (nodesGroupB && nodesGroupB.length>config.sophia.maxResults)
			nodesGroupB.length = config.sophia.maxResults;
	    ...
	    var params = {startNodes: nodesGroupANumbers, 
	...
	{
		// must limit the query size, so cut down on nodes arrays aribtarily		
		if (currentNodes && currentNodes.length > config.sophia.maxResults)
			currentNodes.length = config.sophia.maxResults;

	// TBD:do we need the label checks?
	    var query = "MATCH start-[:"+config.sophia.backboneLinkType+
	    		"*.."+config.sophia.MAX_PATH_LENGTH+"]->backbone" +
				...
				if (results) {
		            results.map(function(result) {
		            	...
		            	result.b.map(function(b) {
		            		bbNodes.push({id:'' + b._id, type: b.labels[0]});
		            	});
		                backbone_nodes.push({test: {id: result.start._id, timestamp: result.start.properties.timestamp}, bbNodes: bbNodes});
		            });
		            if (callback)
	...
	getNodesStats: function(currentNodes, callback) {
		// must limit the query size, so cut down on nodes arrays aribtarily		
		if (currentNodes && currentNodes.length > config.sophia.maxResults)
			currentNodes.length = config.sophia.maxResults;
	    ...
	    var query = "MATCH start-[:"+config.sophia.dataLinkType+"]->next" +
		            ...
		            results.map(function(result) {
		            	result.n.map(function(n) {
		            		if (ids.indexOf(n._id) < 0) {
			            		type = n.labels[0];
			            		if (config.sophia.backboneTypes.indexOf(type) < 0) {
				            		if (!stats[type]) {
                                        stats[type] = [n._id];
				            		} else {
	...
	{
	    var query = "MATCH pathToLaterBackbone = start_node-[:"+config.sophia.backboneLinkType+"*0..]->prev_b" + 
			...
			" MATCH prev_backbone-[:"+config.sophia.backboneLinkType+"*0..1]->next_backbone"+ 
			" WITH prev_backbone, next_backbone ORDER BY next_backbone.timestamp DESC LIMIT 1"+ 
			" MATCH prev_backbone-[:"+config.sophia.dataLinkType+"*0..1]->prev_data"+ 
			" WHERE prev_data.timestamp <= {timestamp} " + 
			" WITH prev_backbone, next_backbone, prev_data ORDER BY prev_data.timestamp DESC LIMIT 1"+ 
			" MATCH nextDataPath=prev_data-[:"+config.sophia.dataLinkType+"*0..1]->next_data"+ 
		...
		prev_data_id, next_data_id, callback)
	{
	    ...
	    if (isBackbone)
		{
		    if (next_backbone_id != prev_backbone_id) {
		        link_query = 'MATCH prev_node, new_node, next_node' +
					...
					' CREATE prev_node-[:' + config.sophia.backboneLinkType + ']' +
					'->new_node-[:' + config.sophia.backboneLinkType + ']->next_node' +
			...
			if (next_data_id != prev_backbone_id)
			{
				...
				if (next_data_id != prev_data_id)
				{
					...
					link_query = link_query + 
						...
						' CREATE new_node-[:'+config.sophia.dataLinkType+']->next_data'+
						' WITH next_data'+
	    ...
	    else // data node
		{
			...
			if (next_data_id != prev_data_id && prev_data_id != prev_backbone_id) {
		        link_query = 'MATCH prev_node, new_node, next_node, prev_backbone' +
					...
					' CREATE prev_node-[:' + config.sophia.dataLinkType + ']' +
					'->new_node-[:' + config.sophia.dataLinkType + ']->next_node' +
					' CREATE prev_backbone-[:' + config.sophia.dataLinkType + ']' +
					'->new_node' +
		...
		console.log(' [****] Linking nodes query:\n' + link_query+
	    	"\nwith params: " + JSON.stringify(params));
	    if (link_query) {
	        db.cypher({query: link_query, params: params}, function(err, results) {
	            if (err) {
	...
	getNearestScreens: function(node_ids, callback){
	    var query = "MATCH node where id(node) IN {node_ids}"+
	    	" OPTIONAL MATCH pathToPrevScreen=(prev:SCREEN)-[:"+config.sophia.dataLinkType+
	    	"*0.."+config.sophia.MAX_PATH_TO_SCREEN_LENGTH+"]->node" +
			" OPTIONAL MATCH pathToNextScreen=node-[:"+config.sophia.dataLinkType+
			"*0.."+config.sophia.MAX_PATH_TO_SCREEN_LENGTH+"]->(next:SCREEN)" +
	    ...
	    db.cypher({query: query, params: params}, function(err, results) {
	        if (err) {
	            ...
	            if (callback && results && results.length>0 && results[0])
	            {
	            	...
	            	callback(results[0].PrevScreenTimestamps, results[0].NextScreenTimestamps);
	            }
				else if (callback)
					callback(new Error('failed to get a new node\'s location'));
				else
