...
var queries = {
	...
	countPaths: function(nodesGroupA, nodesGroupB, callback)
	{
	    ...
	    var query = "MATCH path=start-[*]-destination" +
	            " WHERE id(start) IN ["+ nodesGroupA.join(',')+"]" +
	            " AND id(destination) IN [" + nodesGroupB.join(',') + "]" +
	            " return COUNT(DISTINCT path) as numOfPaths";

	    console.log("Get number of paths query: " + query);
	    db.query(query, null, function(err, results) {
	        if (err) {
	            console.error('neo4j query failed: ' + query + '\n');
               	if (callback)
	...
	getNearestScreens: function(node_ids, callback){
	    var query = "MATCH node where id(node) IN ["+node_ids.join(',')+"]"+
	    	" OPTIONAL MATCH pathToPrevScreen=(prev:SCREEN)-[:"+sophia_consts.dataLinkType+"*0..3]->node" +
			" OPTIONAL MATCH pathToNextScreen=node-[:"+sophia_consts.dataLinkType+"*0..3]->(next:SCREEN)" +
