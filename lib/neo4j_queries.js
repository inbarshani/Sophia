...
var queries = {
	...
	doesPathExists: function(currentStartNodes, destinationNodes, isFirstQuery, callback)
	{
		...
		if (currentStartNodes && currentStartNodes.length>sophia_consts.maxResults)
			currentStartNodes.length = sophia_consts.maxResults;
		if (destinationNodes && destinationNodes.length>sophia_consts.maxResults)
			destinationNodes.length = sophia_consts.maxResults;
		...
		if (isFirstQuery)
		{
			startNode = "(start:"+sophia_consts.backboneRoot+")";
		}
		else
		{
			startNodesWhere = 'id(start) IN {currentStartNodes} AND';
			params.currentStartNodes = currentStartNodes.map(Number);
		}

	    params.destinationNodes = destinationNodes.map(Number);
			
	    var query = "MATCH path=shortestPath("+startNode+"-[*.."+sophia_consts.MAX_PATH_LENGTH+"]->destination)" +
	            " WHERE " +
	            startNodesWhere +
	            " id(destination) IN {destinationNodes}" +
	...
	countPaths: function(nodesGroupA, nodesGroupB, callback)
	{
		...
		if (nodesGroupA && nodesGroupA.length>sophia_consts.maxResults)
			nodesGroupA.length = sophia_consts.maxResults;
		if (nodesGroupB && nodesGroupB.length>sophia_consts.maxResults)
			nodesGroupB.length = sophia_consts.maxResults;
			
	    var query = "MATCH path=shortestPath(start-[*]-destination)" +
	            " WHERE id(start) IN {startNodes}" +
	    ...
	    console.log("Get number of paths query: " + query+" with params: " + JSON.stringify(params));
	...
	{
		// must limit the query size, so cut down on nodes arrays aribtarily		
		if (currentNodes && currentNodes.length > sophia_consts.maxResults)
			currentNodes.length = sophia_consts.maxResults;

	// TBD:do we need the label checks?
	    var query = "MATCH start-[:"+sophia_consts.backboneLinkType+
	    		"*.."+sophia_consts.MAX_PATH_LENGTH+"]->backbone" +
	            " WHERE id(start) IN {currentNodes}" + 
	...
	// TBD:do we need the label checks?
	    var query = "MATCH start-[:"+sophia_consts.dataLinkType+
	    		"*.."+sophia_consts.MAX_PATH_LENGTH+"]->next" +
	            " WHERE id(start) IN ["+currentNodes.join(',')+"]" + 
	...
	getNodeLocationByTimestamp: function(test_id, timestamp, callback)
	{
	    var query = "MATCH pathToLaterBackbone = start_node-[:"+sophia_consts.backboneLinkType+"*0..]->prev_b" + 
			...
			" MATCH prev_backbone-[:"+sophia_consts.dataLinkType+"*0..1]->prev_data"+ 
			" WHERE prev_data.timestamp <= {timestamp} " + 
			" WITH prev_backbone, next_backbone, prev_data ORDER BY prev_data.timestamp DESC LIMIT 1"+ 
			" MATCH nextDataPath=prev_data-[:"+sophia_consts.dataLinkType+"*0..1]->next_data"+ 
	    ...
	    else // data node
		{
			...
			if (next_data_id != prev_data_id && prev_data_id != prev_backbone_id) {
		        link_query = 'MATCH prev_node, new_node, next_node, prev_backbone' +
					...
					' CREATE prev_backbone-[:' + sophia_consts.dataLinkType + ']' +
					'->new_node' +
	...
	getNearestScreens: function(node_ids, callback){
	    var query = "MATCH node where id(node) IN {node_ids}"+
	    	" OPTIONAL MATCH pathToPrevScreen=(prev:SCREEN)-[:"+sophia_consts.dataLinkType+
	    	"*0..3]->node" +
			" OPTIONAL MATCH pathToNextScreen=node-[:"+sophia_consts.dataLinkType+
			"*0..3]->(next:SCREEN)" +
