...
var queries = {
	...
	getNearestScreens: function(node_id, callback){
	    var query = "MATCH node where id(node)="+node_id+
	    	" OPTIONAL MATCH pathToPrevScreen=(prev:SCREEN)-[:"+sophia_consts.dataLinkType+"*]->node" +
			" OPTIONAL MATCH pathToNextScreen=node-[:"+sophia_consts.dataLinkType+"*]->(next:SCREEN)" +
			" return prev.timestamp AS PrevScreenTimestamp, next.timestamp AS NextScreenTimestamp LIMIT 1";
	    ...
	    db.query(query, null, function(err, results) {
	        if (err) {
	            ...
	            if (callback && results && results.length>0 && results[0])
	            {
	            	...
	            	callback(results[0].PrevScreenTimestamp, results[0].NextScreenTimestamp);
	            }
				else if (callback)
					callback(new Error('failed to get a new node\'s location'));
				else
