...
Object.keys(sophia_app_config).map(function(i) { 
	if (sophia_app_config[i].constructor === Object) {
		if (sophia_app_config[i][0]) {
			sophia_app_config[i] = Object.keys(sophia_app_config[i]).map(function(j) { return sophia_app_config[i][j] });
		}
	}
});
