...
var configResolveCount = (sophia_app_config.dnsResolveVars ? 
	sophia_app_config.dnsResolveVars.length : 0);
...
Object.keys(sophia_app_config).map(function(key) { 
	...
	if (sophia_app_config.dnsResolveVars && 
		sophia_app_config.dnsResolveVars.indexOf(key) >= 0)
	{
		require('dns').lookup(sophia_app_config[key], function(err, address, family){
			if (!err)				
			{
				...
				sophia_app_config[key+'_RESOLVED'] = address;
			}
			configResolveCount--;
		});
	}
...
sophia_app_config.ready = function(callback){
	var intervalID = setInterval(function(){
		if (configResolveCount == 0)
		{
			clearInterval(intervalID);
			if (callback)
				callback(sophia_app_config);
		}
	}, 50);
}
