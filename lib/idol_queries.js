        ...
        if (results) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (result_type == 'ui_objects')
                    formatted_result = ui_objects.extractDataFromIDOL(idol_document, 
...
sophia_config.ready(function(){
    queries.idol_url = 'http://' + sophia_config.IDOL_SERVER + ':9000/action=';
});
...
var queries = {
    ...
    searchReview: function(query, dateCondition, callback) {
        ...
        var url_query = this.idol_url + 'Query&Text=' + query_phrase + 
            '&FieldText=MATCH{Test}:SOPHIATYPE' +
            '&databasematch=' + sophia_config.IDOL_DB_NAME +
            '&maxResults=' + this.idol_maxResults +
            '&MinScore=' + this.idol_weight_threshold +
            getDateConditionString(dateCondition) +
    ...
    formatUIObjectAttributeQuery: function(attributes, terms)
    {
        // YNEAR - Weighted NEAR (with AND operation). Proximity operator that 
        //  returns documents that contain both of the terms. It promotes relevance 
        //  when term spacing is less than the specified N word distance 
        //  (closer together implies higher relevance).
        var query = '';
        var that = this;
        if (terms)
        {
            attributes.forEach(function(attribute){
                ...
                for(var i=0;i<terms.length;i++)
                {
                    ...
                    {
                        console.log('formatUIObjectAttributeQuery Convert color to rgb, original: '+term);
                        // need to translate into rgb
                        if (term.match(/^#/))
                            term = that.hexToRgb(term);
                        else // translate color name into hex first
                        {
                            var colors_hex = Object.keys(sophia_config.color_palette);
                            for(var j=0;j<colors_hex.length;j++)
                            {
                                if (sophia_config.color_palette[colors_hex[j]] == term)
                                {
                                    term=that.hexToRgb(colors_hex[j]);
                                    break;
                                }
                            }                    
                        }
                        console.log('formatUIObjectAttributeQuery Convert color to rgb, result: '+term);
                    }
                    query += '(' + attribute + ' YNEAR'+sophia_config.uiObjectsAttributeWordLimit 
                        + ' ' + term + ')';
                    if (i<terms.length - 1)
                        query += ' OR ';
                }
    ...
    searchUIObjects: function(patternsObj, dateCondition, callback) {
        var query = '(';
        query += this.formatUIObjectAttributeQuery(['font_family'], patternsObj.fontSearches);
        if (query.length > 1 && patternsObj.styleSearches && 
            patternsObj.styleSearches.length > 0)
            query += ') AND (';
        query += this.formatUIObjectAttributeQuery(['classNames'], patternsObj.styleSearches);
        if (query.length > 1 && patternsObj.colorSearches && 
            patternsObj.colorSearches.length > 0)
            query += ') AND (';
        query += this.formatUIObjectAttributeQuery(['color', 'background'], patternsObj.colorSearches);
        query += ')';
        console.log('searchUIObjects combined text query: ' + query);

        var query_phrase = fixedEncodeURIComponent(query);

        var url_query = this.idol_url + 'Query&Text=' + query_phrase +
            '&FieldText=MATCH{'+sophia_config.uiObjectsTypes.join(',')+'}:SOPHIATYPE' +
            '&databasematch=' + sophia_config.IDOL_DB_NAME +
            '&maxResults=' + this.idol_maxResults +
            '&MinScore=' + this.idol_weight_threshold +
            getDateConditionString(dateCondition) +
        ...
        var http_req = http.get(url_query, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL search for ' + url_query + ' completed: ' +
                    res.statusCode); // + '\ndata: ' + response_data);
                // extract node data from response XML
                handleSearchResults(response_data, false, false,callback);
            });
        });
    },
