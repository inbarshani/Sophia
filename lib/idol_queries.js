...
function handleSearchResults(response_data, resultsWithHash, callback) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (result_type.indexOf('ui_') == 0)
                    ...
                    formatted_result = ui_logical.extractDataFromIDOL(idol_document, 
                        formatted_result, resultsWithHash);
...
var queries = {
    ...
    search: function(query, dateCondition, callback) {
        ...
        var http_req = http.get(url_query, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL search for ' + url_query + ' completed: ' +
                    res.statusCode); // + '\ndata: ' + response_data);
                handleSearchResults(response_data, false, callback);
            });
        });
    },

    searchByReference: function(referencesArray, resultsWithHash, callback) {
        ...
        var http_req = http.get(url_query, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL search for ' + url_query + ' completed: ' +
                    res.statusCode); // + '\ndata: ' + response_data);
                handleSearchResults(response_data, resultsWithHash, callback);
            });
        });
    },
    ...
    analyzeImagePost: function(filePath, callback) {
        ...
        var post_req = http.request(post_options, function(res) {
            ...
            console.log("analyzeImagePost got error: " + e.message);
            callback(null);
        });
    ...
    analyzeImageCheck: function(token, callback){
        ...
        var req = http.get(http_options, function(res) {
            ...
            console.log("analyzeImageCheck got error: " + e.message);
            callback(null);
        });
    },
    ...
    searchSimilar: function(testStepID, dateCondition, callback) {
        // since the IDOL 'Suggest' REST is not good enough,
        //  we first fetch the origianl step HASH, so we can screen results
        //  after IDOL query
        var url_query = this.idol_url + 'Suggest&Reference=' + testStepID +
        ...
        this.searchByReference([testStepID], true, function(documents) {
            //console.log('[!!!] searchSimilar testStep:\n' + JSON.stringify(documents['' + testStepID]));
            var testStepHash = documents['' + testStepID].hash;            
            ...
            var http_req = http.get(url_query, function(res) {
                ...
                res.on('end', function() {
                    ...
                    xml2js(response_data, function(err, search_results) {
                        ...
                        //console.log(require('util').inspect(results));
                        var formatted_results = {};
                        var testID = documents['' + testStepID].testID;
                        formatted_results['' + testID] = [documents['' + testStepID]];
                        if (results) {
                            console.log('[!!] IDOL search number of results: ' + results.length);
                            for (var i = 0; i < results.length; i++) {
                                var result = results[i];
