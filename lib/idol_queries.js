...
//        var post_data = 'ImageData=' + data;
        ...
        var post_req = http.request(post_options, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL OCR POST completed: ' +
                    res.statusCode + '\ndata: ' + response_data);
                xml2js(response_data, function(err, search_results) {
                    var text = '';
                    if (search_results.autnresponse.action[0] == 'ANALYZE') {
                        if (search_results.autnresponse.responsedata[0].error) {
                            ...
                            var page = search_results.action[0].page;
                            if (page.ocrtask.textblock != null) {
                                for (var i = 0; i < page.ocrtask.textblock.length; i++) {
                                    if (page.ocrtask.textblock[i] != null) {
                                        text += page.ocrtask.textblock[i].text + "\n";
                                    }
                                }
                            }
                        }
