        ...
        if (results) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (result_type.indexOf('ui_') == 0)
                    ...
                    formatted_result = screen.extractDataFromIDOL(idol_document, formatted_result,isExpendedData);
    ...
    searchByReference: function(referencesArray, resultsWithHash, callback) {
        ...
        var http_req = http.get(url_query, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL search for ' + url_query + ' completed: ' +
                    res.statusCode); // + '\ndata: ' + response_data);
                handleSearchResults(response_data, resultsWithHash,false, callback);
            });
        });
    },
            ...
            if (key != 'type' && key != 'indexable_content' &&
                key != 'high_priority_index' && key != 'dontStore')
                {
                    ...
                    if (typeof data[key] === 'object')
                        key_val = JSON.stringify(data[key]);
                    else
                        key_val = data[key].toString();
                    post_data = post_data + '#DREFIELD ' + key + '=\"' + 
                        encodeURIComponent(key_val) + '\"\r\n';
                }
        });
