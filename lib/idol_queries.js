...
function handleSearchResults(response_data, callback) {
...
var queries = {
        ...
        var http_req = http.get(url_query, function(res) {
            ...
            res.on('end', function() {
                console.log('[!!] IDOL search for ' + url_query + ' completed: ' +
                    res.statusCode); // + '\ndata: ' + response_data);
                handleSearchResults(response_data, callback);
            });
        });
    },

    searchByReference: function(referencesArray, callback) {
    ...
    updateIdolDocument: function(doc_id, data) {
        ...
        var post_options = {
            host: '16.60.168.105',
            port: '9001',
            path: '/DREREPLACE?DATABASEMATCH='+sophia_consts.IDOL_DB_NAME,
            method: 'POST',
            headers: {
                'Content-Type': 'text/plain',
                'Content-Length': post_data.length
